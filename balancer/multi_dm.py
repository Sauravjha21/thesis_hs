...
class MultiDM(Balancer):
    def __init__(self, N, D, delta=0.05, q=0.5, intercept=True, **kwargs):
        """
        Args:
        ...
        for cur_node in range(num_nodes)[::-1]:
            ...
            if self.nodes[left] is None or self.nodes[right] is None:
                ...
                self.nodes[parent] = DM(
                    N=N, D=D, intercept=intercept, delta=delta, q=pr_right
                )
                self.weights[parent] = left_weight + right_weight

    def assign_next(self, x):
        cur_idx = 0
        while isinstance(self.nodes[cur_idx], DM):
